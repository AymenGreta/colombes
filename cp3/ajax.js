/*
* Branche l'évènement LOAD à WINDOW
*/
window.addEventListener(
    'load',
    function(){
        //Etape 1 : Instanciation de la requête AJAX
        let oXHR = new XMLHttpRequest;
        //Etape 2 : Ouverture requête AJAX
        oXHR.open('get','https://restcountries.eu/rest/v2/all', true);
        //Etape 3 : Envoi requête AJAX
        oXHR.send();
        //Etape 4 : Traitement retour du serveur
        oXHR.addEventListener(
        'readystatechange',
        function(){
            if(oXHR.status === 200 && oXHR.readyState === 4)
            {
                //Transforme la réponse text en objet
...
function getCity(){
    ...
    let oXHR = new XMLHttpRequest;
    oXHR.open('get','https://www.zippopotam.us/' + sLand + '/' + sZip, true);
    oXHR.send();
    oXHR.addEventListener(
        'readystatechange',
        function(){
